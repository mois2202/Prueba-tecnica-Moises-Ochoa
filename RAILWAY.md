...
* **Frontend**: Nixpacks detecta la configuración, inyecta la variable de entorno `VITE_API_URL` para que Vite la incorpore en el bundle final, ejecuta `npm run build` (`tsc -b && vite build`) y, al detectar el archivo `Caddyfile` sin script de inicio `start` en `package.json`, levanta de forma nativa **Caddy** como servidor web de producción de alto rendimiento. El archivo `Caddyfile` se encarga de habilitar la compresión Gzip y la directiva `try_files {path} /index.html` para garantizar que cualquier recarga (F5) en las rutas del frontend (como `/dashboard` o `/login`) funcione correctamente sin producir errores 404.
